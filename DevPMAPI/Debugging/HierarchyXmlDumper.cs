...
using MelonLoader;
using PMAPI.Debugging.ComponentDumpers;
using System.IO;
using System.Text;
using System.Xml;
using UnityEngine;
...
namespace PMAPI.Debugging
{
	...
	public static class HierarchyXmlDumper
	{
		...
		public static ComponentDumperList DefaultDumperList = new ComponentDumperList(
            ...
            //new ElectricPartComponentDumper(),
            new BatteryComponentDumper(),
            new LEDComponentDumper(),
            new PyriteComponentDumper(),
            new TungstenComponentDumper()
		...
		{
			MelonLogger.Msg("Starting HierarchyDump Dump");
			XmlDocument document = new XmlDocument();
			DumpScene(scene, document, dumperList);

			document.Save(FilePath);
			MelonLogger.Msg($"Dump complete saved at '{Path.Combine(Il2CppSystem.Environment.CurrentDirectory, FilePath)}'");
		}
